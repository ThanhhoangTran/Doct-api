...
export class MessageProcessorService {
  ...
  public async process(payload: ProcessMessagePayload): Promise<void> {
    try {
      ...
      await Promise.all(receiverConnections.map(connection => gateway.sendToConnection(connection.connectionId, ActionResponse(ROUTE_KEY.MESSAGE, chatMessage.toObject()))));
    } catch (error) {
